...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint){
		...
		for(Marker mk : this.quakeMarkers) {
			if (mk instanceof EarthquakeMarker) {
				EarthquakeMarker valMk = (EarthquakeMarker) mk;
				quakeList.add(valMk);
			}
		}
	...
	private void topEarthquake(CityMarker city, List<EarthquakeMarker> quakeList){
		...
		Collections.sort(quakeList);
		System.out.print(quakeList.size() + "\n");
		if(quakeList.size() == 0){
			this.rank[0] = "N/A";
			return;
		}
		for(int i = 0; i < numToPrint; i++){
			if(i < quakeList.size()) {
				this.rank[i] = String.valueOf(quakeList.get(i).getMagnitude() + " " +
						quakeList.get(i).getStringProperty("age"));
			}else{
				this.rank[i] = "";
			}
		}
			...
			if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				...
				for (Marker mhide : quakeMarkers) {
					...
					if (quakeMarker.getDistanceTo(marker.getLocation()) 
						...
						quakeList.add(quakeMarker);
					}
				}
				topEarthquake((CityMarker)marker, quakeList);
				return;
			}
		}		
	}
	...
	private void addKey() {	
		...
		rect(25, 420, 140, 200);
		fill(0, 0, 0);
		text("Top 5 earthquakes:", 35, 460);
		text(this.rank[0], 45, 500);
		text(this.rank[1], 45, 520);
		text(this.rank[2], 45, 540);
		text(this.rank[3], 45, 560);
		text(this.rank[4], 45, 580);
	}
